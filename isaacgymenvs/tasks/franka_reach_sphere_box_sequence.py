...
class FrankaReachSphereBoxSequence(VecTask):
    def __init__(self, cfg, rl_device, sim_device, graphics_device_id, headless, virtual_screen_capture, force_render):
        ...
        self.scales = [1.0, 0.8, 0.6, 0.4, 0.2]
        self.curr_step = 0
        self.scale = self.scales[self.curr_step]
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        for scale in self.scales:
            # asset_options.scale = scale
            box_assets[scale] = self.gym.load_asset(self.sim, asset_root, box_asset_file, asset_options)
        init_box_asset = box_assets[self.scale]
        ...
        for i in range(self.num_envs):
            ...
            box_actor = self.gym.create_actor(env_ptr, init_box_asset, box_pose, "box", i, 1, 0)
            self.gym.set_actor_scale(env_ptr, box_actor, self.scale)
            ...
            ball_height = round(0.5 * self.scale, 2)
            ball_pose.p = gymapi.Vec3(*get_axis_params(ball_height, self.up_axis_idx))
    ...
    def change_box_size(self, new_scale):
        self.box_scale = new_scale
        for i in range(self.num_envs):
            box_handle = self.boxes[i]
            self.gym.set_actor_scale(self.envs[i], box_handle, self.box_scale)

            ball_handle = self.ball_handles[i]
            ball_height = 0.5 * self.box_scale
            ball_pose = gymapi.Transform()
            ball_pose.p = gymapi.Vec3(*get_axis_params(ball_height, self.up_axis_idx))
            success = self.gym.set_rigid_transform(self.envs[i], ball_handle, ball_pose)

            # if not success:
            #     print(f"Failed to set updated ball position. Success: {success}")
    ...
    def compute_reward(self, actions):
        ...
        # print(f"Sum of rewards: {torch.sum(self.rew_buf)}")
        # print(f"No. of resets: {torch.sum(self.reset_buf)}")
    ...
    def reset_idx(self, env_ids):
        ...
        if torch.sum(self.rew_buf) > 4 * self.num_envs and self.curr_step < len(self.scales) - 1:
            self.curr_step += 1
            self.scale = self.scales[self.curr_step]
            self.change_box_size(self.scale)
...
def compute_franka_reward(
    ...
    # If the arm is very close to the ball, give a reward
    bonus_dist_reward = torch.where(d <= 0.05, 1.0, 0.0)
    ...
    rewards = (dist_reward * reward_settings['r_dist_reward_scale'] -             # Max: 10.0
               action_penalty * reward_settings['r_action_penalty_scale'] +
               open_gripper_reward * reward_settings['r_open_reward_scale'] +     # Max: 1.0
               bonus_dist_reward * reward_settings['r_bonus_dist_reward_scale'])  # Max: 1.0

    # print(f"Rewards: {rewards[0]}\nDistance Reward: {dist_reward[0]}\nAction Penalty: {action_penalty[0]}\nOpen Gripper Reward: {open_gripper_reward[0]}\nBonus Distance Reward: {bonus_dist_reward[0]}\nAction Penalty: {action_penalty[0]}")
